...
export function createApolloClient(context?: GetServerSidePropsContext) {
  ...
  const errorLink = onError(({ networkError, graphQLErrors }) => {
    ...
    // IMPORTANT: Skip credential clearing on /login page — login mutations (loginByEmail,
    // passkeyLoginOptions, etc.) return UNAUTHENTICATED errors as part of normal flow
    // (e.g. EMAILKEY_REQUIRED). Clearing credentials here would nuke a valid JWT from
    // a previous session or OAuth login, causing the user to get logged out.
    if (graphQLErrors) {
      for (const error of graphQLErrors) {
        if (error.extensions?.code === 'UNAUTHENTICATED' ||
            error.message?.toLowerCase().includes('unauthorized')) {
          console.error('[Apollo] GraphQL UNAUTHENTICATED error:', error.message)
          if (isBrowser && !window.location.pathname.includes('/login')) {
            console.error('[Apollo] Clearing credentials and redirecting to login')
            Cookies.remove(jwtKey, { path: '/' })
            localStorage.removeItem('jwt_fallback')
            localStorage.removeItem('didToken')
            jwt = undefined
