 ...
 *
 * Vercel-direct (Phase 7e Apollo strip) — returns the original Post that
 * was authored to share a given track (Track → originating Post lookup,
 * used by track-detail "View Post" + Social card).
 *
 * Shape mirrors /api/feed/post (PostComponentFields). Returns
 * { post: <PostComponentFields | null> }.
 */
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Find earliest post that references this trackId (the "original" share)
    const post: any = await db.collection('posts')
      .find({ trackId: trackOid, deleted: { $ne: true } })
      .sort({ createdAt: 1 })
      .limit(1)
      .next()

    if (!post) return res.status(200).json({ post: null })
    ...
    const [author, track, myReaction, myBookmark] = await Promise.all([
      ...
      db.collection('tracks').findOne({ _id: trackOid }),
      viewerProfileId
        ? db.collection('reactions').findOne({ profileId: viewerProfileId, postId: post._id }, { projection: { type: 1 } as any })
        : Promise.resolve(null),
      viewerProfileId
        ? db.collection('bookmarks').findOne({ profileId: viewerProfileId, postId: post._id }, { projection: { _id: 1 } as any })
        : Promise.resolve(null),
    ])
