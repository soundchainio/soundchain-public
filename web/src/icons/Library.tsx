...
export const Library = ({ color, ...props }: IconProps) => {
  return (
    <svg width={20} height={20} fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
      <rect
        ...
        height={14}
        rx={3}
        stroke={color ? `url(#${color}-gradient)` : '#505050'}
        strokeWidth={2}
      />
      <path
      ...
      <rect x={4} y={2} width={12} height={1.5} rx={0.75} fill={color ? `url(#${color}-gradient)` : '#505050'} />
      <rect x={6} width={8} height={1.5} rx={0.75} fill={color ? `url(#${color}-gradient)` : '#505050'} />
    </svg>
  );
};
