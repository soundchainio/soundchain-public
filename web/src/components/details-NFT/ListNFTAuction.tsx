    ...
    <div className="mb-2 pb-16">
      <Formik
        ...
        onSubmit={(values, helper) => {
          handleSubmit(
            { ...values, startTime: new Date(values.startTime), endTime: new Date(values.endTime), isPaymentOGUN },
            helper,
          );
        }}
      >
        {({ values, errors, isSubmitting, setFieldValue }: FormikProps<ListNFTAuctionFormValues>) => (
          <Form>
            ...
            <div className="flex items-center justify-between bg-gray-20 py-3 px-5">
              <label
                htmlFor="endTime"
                className={`flex items-center justify-start  text-xs font-bold text-gray-80 ${config.mobileBreakpoint}:text-sm `}
              >
                <div className="mr-3 flex flex-col">
            ...
            <div className="bg-gray-20 py-3 px-5">
              <SoundchainFee price={values.price} isPaymentOGUN={values.isPaymentOGUN} />
            </div>
