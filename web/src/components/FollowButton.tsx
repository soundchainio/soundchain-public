...
export const FollowButton = ({ followedId, isFollowed, ...rest }: FollowButtonProps) => {
  ...
  const handleClick = async () => {
    ...
    if (!me) {
      router.push('/login', { query: { redirect_url: window.location.href } });
      return;
    }
  ...
  return (
    <Button onClick={handleClick} variant="rainbow-rounded" className="w-20 bg-gray-10 text-sm" {...rest}>
