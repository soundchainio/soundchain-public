...
export const BottomAudioPlayer = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex flex-col bg-neutral-900 py-1.5 px-3 pb-safe pl-safe pr-safe animate-slide-up shadow-[0_-4px_20px_rgba(0,0,0,0.5)] box-border">
      ...
      <div className="flex items-center justify-between gap-2 w-full mt-0.5">
        ...
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-full bg-white/10 active:bg-white/20 flex-shrink-0"
          onClick={closePlayer}
        >
        ...
        >
          <div className="relative flex-shrink-0 w-9 h-9 rounded-md overflow-hidden bg-gray-800">
            <Asset src={currentSong.art} sizes="36px" />
          </div>
          <div className="flex flex-col items-start min-w-0 flex-1 overflow-hidden">
            <h2 className="text-[13px] font-bold text-white truncate w-full leading-tight">{currentSong.title || 'Unknown title'}</h2>
            <p className="text-[11px] text-gray-400 truncate w-full leading-tight">{currentSong.artist || 'Unknown artist'}</p>
          </div>
        </button>
