...
export const MusicianTypeSelector = ({ onSelect, maxSelections = 5, initialValue }: MusicianTypeSelectorProps) => {
  ...
  return (
    <div className="flex flex-col">
      <Label>What type of musician you are? {selectedMusicianTypes.length ? `(${selectedMusicianTypes.length} Selected)` : ''}</Label>
      <div className="pb-6 space-y-2">
        {musicianTypes.map(({ label, key }) => (
          <Badge key={key} label={label} selected={isMusicianTypeSelected(key)} onClick={() => onMusicianTypeClick(key)} />
        ))}
      </div>
    </div>
  );
