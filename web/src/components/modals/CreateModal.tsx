...
export const CreateModal = () => {
  ...
  const { web3: magicWeb3, account: magicAccount, magic } = useMagicContext()
  ...
  // Pin files/JSON to IPFS via Vercel API routes (bypasses Lambda)
  const pinFileToIPFS = async (fileUrl: string, fileName: string): Promise<string> => {
    const resp = await fetch('/api/ipfs/pin-file', {
      method: 'POST',
    ...
    if (!resp.ok) {
      ...
      throw new Error(err.error || `IPFS pin failed: ${resp.status}`)
    }
    const result = await resp.json()
    return result.cid
  }

  const pinJsonToIPFSViaVercel = async (json: unknown, fileName: string): Promise<string> => {
    const resp = await fetch('/api/ipfs/pin-json', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({ error: `HTTP ${resp.status}` }))
      throw new Error(err.error || `IPFS JSON pin failed: ${resp.status}`)
    }
    const result = await resp.json()
    return result.cid
  }
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    if (file && web3 && account) {
      ...
      try {
        ...
        const assetCid = await pinFileToIPFS(assetUrl, values.title)
        ...
        if (artworkFile) {
          ...
          const artCid = await pinFileToIPFS(artworkUrl, `${title}-art`)
          metadata.art = `ipfs://${artCid}`
          metadata.image = `ipfs://${artCid}`
        }

        const metadataCid = await pinJsonToIPFSViaVercel(metadata, `${title}-metadata`)
          ...
          return new Promise((resolve, reject) => {
            createEdition(account, account, royalty, editionQuantity, Number(nonce)) // Convert bigint to number
              .onReceipt(async receipt => {
                ...
                console.log('[Mint] Receipt received. status:', receipt.status, 'events:', receipt.events ? Object.keys(receipt.events) : 'none', 'logs:', (receipt as any).logs?.length)
                const editionCreated = receipt.events?.['EditionCreated']
                const editionId = Number(editionCreated?.returnValues?.editionNumber)
                console.log('[Mint] EditionCreated event:', editionCreated, 'editionId:', editionId)
                if (!editionCreated || !Number.isFinite(editionId) || editionId <= 0) {
                  console.error('[Mint] Failed to decode EditionCreated event from receipt. Full receipt:', JSON.stringify(receipt, null, 2))
                  reject(new Error('Could not read edition ID from blockchain receipt. Transaction succeeded but event decoding failed.'))
                  return
                }
        ...
        if (platformFee > 0 && config.treasuryAddress) {
          ...
          try {
            ...
            // Use ethers.js for signing — web3.js v4 sendTransaction() throws
            // LocalWalletNotAvailableError because it can't find the account in its local wallet.
            // Magic's documented signing path is ethers.providers.Web3Provider(magic.rpcProvider).
            const rpcProvider = (magic as any)?.rpcProvider || (window as any).ethereum
            if (!rpcProvider) {
              throw new Error('No wallet provider available for fee transaction')
            }
            const ethersProvider = new ethers.providers.Web3Provider(rpcProvider)
            const signer = ethersProvider.getSigner()
            const feeTx = await signer.sendTransaction({
              to: treasuryAddress,
              value: ethers.BigNumber.from(feeInWei.toString()),
              gasLimit: ethers.BigNumber.from(100000),
              gasPrice: ethers.BigNumber.from(gasPrice.toString()),
            })
            await feeTx.wait()

            console.log(`✅ Platform fee sent! TX: ${feeTx.hash}`)
      ...
      {tab === Tabs.SCID ? (
        ...
        <>
          ...
          <div className="px-4 pt-4">
            {/* Show recommendation if only Magic wallet available AND Web3Modal is ready */}
            {hasMagicWallet && !hasExternalWallet && isWeb3ModalReady && (
              <div className="mb-3 p-3 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
                ...
                <p className="text-xs text-gray-400 mb-2">
                  External wallets mint faster with no rate limits. Or just proceed below with your OAuth wallet.
                </p>
                <button
                  onClick={() => {
                  ...
                  className="w-full text-sm px-4 py-2.5 rounded-lg font-medium transition-all flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white"
                >
