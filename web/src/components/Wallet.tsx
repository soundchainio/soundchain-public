...
export const Wallet = ({ title, correctNetwork, account, balance, icon: Icon, defaultWallet, showActionButtons }: WalletProps) => {
  return (
    <>
      ...
      {correctNetwork &&
        <>
          <div className="flex flex-row bg-gray-1A w-full px-6 h-14 items-center justify-between">
            <div className="flex flex-row items-center w-9/12">
              <LinkIcon />
              <span className="text-gray-80 text-sm mx-1 truncate w-full">{account}</span>
            </div>
            <div className="flex justify-end w-3/12">
              <button className="flex flex-row items-center border border-gray-30 border-opacity-75 rounded-sm p-1" onClick={() => { navigator.clipboard.writeText(account + '') }}>
                <span className=""><Copy /></span>
                <span className="text-gray-80 text-xs pl-1 uppercase leading-tight">copy</span>
              </button>
            </div>
          </div>
          <div className="flex w-full">
            <div className="flex items-center justify-center w-7/12 bg-gray-20 h-20 text-gray-CC font-bold text-xs uppercase">Total Balance</div>
            <div className="flex flex-wrap items-center justify-center w-5/12 bg-gray-30 h-20 uppercase">
            ...
            <div className="flex w-full justify-around p-6">
              <Link href="/" passHref>
                <button className="bg-gray-20 py-2 w-40">
                  <span className="uppercase green-gradient-text text-sm">Buy tokens</span>
                </button>
              </Link>
              <Link href="/" passHref>
                <button className="bg-gray-20 py-2 w-40">
                  <span className="uppercase yellow-gradient-text text-sm">Send tokens</span>
                </button>
              </Link>
            </div>
          }
