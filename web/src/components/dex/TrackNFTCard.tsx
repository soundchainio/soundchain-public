...
import { Play, Pause, Heart, RotateCcw, X, Maximize2, Share2, ShoppingCart, ExternalLink, Music2, Zap, Layers } from 'lucide-react'
                  ...
                  {track.nftData && (
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        {(track.editionSize ?? 0) > 1 && (
                          <div className="flex justify-between bg-gray-800/40 rounded px-2 py-1">
                            <span className="text-gray-500">Edition</span>
                            <span className="text-purple-400 font-mono">×{track.editionSize}</span>
                          </div>
                        )}
                        <div className="flex justify-between bg-gray-800/40 rounded px-2 py-1">
    ...
    <div className="track-nft-card-container cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
      <div className={`nft-flip-card ${isFlipped ? 'flipped' : ''}`}>
        ...
        <div className="nft-flip-card-front">
          <Card className={`retro-card transition-all duration-300 ease-out hover:scale-[1.03] hover:shadow-xl hover:shadow-cyan-500/20 h-full border bg-gradient-to-b from-gray-900 to-gray-950 rounded-xl overflow-hidden backdrop-blur-sm ${isCurrentTrack ? 'ring-2 ring-cyan-400 shadow-lg shadow-cyan-500/30 border-gray-700/50' : isOwned ? 'ring-2 ring-yellow-400 shadow-[0_0_16px_rgba(250,204,21,0.5),0_0_6px_rgba(250,204,21,0.3)] border-yellow-400/50' : isCreatorOnly ? 'ring-2 ring-slate-300/70 shadow-[0_0_12px_rgba(148,163,184,0.4),0_0_4px_rgba(148,163,184,0.3)] border-slate-400/30' : 'border-gray-700/50'}`}
            ...
            <div className="aspect-square bg-gray-800 overflow-hidden relative group">
              ...
              {/* Edition badge - shows edition size for multi-edition NFTs */}
              {(track.editionSize ?? 0) > 1 && (
                <Badge className={`bg-purple-600/90 text-white absolute ${hasListing ? 'top-7' : 'top-1.5'} left-1.5 text-[8px] px-1.5 py-0.5 font-bold tracking-wide backdrop-blur-sm shadow-sm flex items-center gap-0.5`}>
                  <Layers className="w-2.5 h-2.5" /> ×{track.editionSize}
                </Badge>
              )}
            ...
            <div className="flex-1 overflow-y-auto space-y-1.5 text-[9px] custom-scrollbar-mini pr-1">
              ...
              {track.nftData?.tokenId && (
                <div className="metadata-section-compact">
                  ...
                  {(track.editionSize ?? 0) > 1 && (
                    <div className="flex justify-between"><span className="text-gray-500">EDITION:</span><span className="text-purple-400">×{track.editionSize}</span></div>
                  )}
...
export const TrackNFTCard = memo(TrackNFTCardComponent, (prevProps, nextProps) => {
  ...
  return (
    ...
    prevProps.track.editionSize === nextProps.track.editionSize
  )
})
