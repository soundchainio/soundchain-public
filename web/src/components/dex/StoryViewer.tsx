...
// MakeStoryPermanentModal removed — feature disabled
...
const RECORD_STORY_WATCH = gql`
  mutation recordStoryWatch($storyId: String!, $watchDurationSeconds: Float!) {
...
interface StoryItem {
  ...
  caption?: string // embed URL (YouTube/Spotify) or text caption
}
...
export const StoryViewer = ({ isOpen, onClose, initialUserId, initialStoryId, users = mockStoryUsers }: StoryViewerProps) => {
  ...
  // Detect embed URL in caption (YouTube/Spotify/etc. added via Music tab)
  const captionEmbedUrl = currentStory?.caption && isEmbedUrl(currentStory.caption) ? currentStory.caption : null
  const hasEmbedAudio = !!captionEmbedUrl && !isEmbedUrl(currentStory?.mediaUrl || '')
  // For stories with attached track audio: use story duration (59s from radio) or default 59s
  // For embed audio (YouTube in caption): use detected video duration or story duration
  ...
  const storyDuration = hasTrackAudio
    ...
    : hasEmbedAudio
    ? (currentStory?.duration ? currentStory.duration * 1000 : videoDuration || 59000)
  ...
  useEffect(() => {
    if (!audioRef.current) return
    console.log('[StoryViewer] Audio check:', { hasTrackAudio, hasEmbedAudio, captionEmbedUrl, attachedTrack: currentStory?.attachedTrack, storyId: currentStory?.id })
    if (hasTrackAudio) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const elapsed = accumulatedTime + (Date.now() - startTime - pausedAt)
    ...
    const handleStoryVisibility = () => {
      if (document.hidden) {
        if (progressInterval.current) clearInterval(progressInterval.current)
        pausedAt = Date.now() - startTime - accumulatedTime
      } else {
        accumulatedTime += 0
        progressInterval.current = setInterval(tick, 50)
      }
    }
    document.addEventListener('visibilitychange', handleStoryVisibility)

    return () => {
      if (progressInterval.current) clearInterval(progressInterval.current)
      document.removeEventListener('visibilitychange', handleStoryVisibility)
    }
  ...
  const handleReaction = async (emoji: string) => {
    if (!currentStory?.id) return
    // Optimistic UI - show reacted immediately with visual flash
    ...
    try {
      ...
      toast.success(`Reacted ${emoji}`, { autoClose: 2000, hideProgressBar: true, style: { fontSize: '1.25rem', zIndex: 9999 } })
    } catch (err: any) {
      // Remove optimistic reaction on error
      setReactedEmojis(prev => { const next = new Set(prev); next.delete(emoji); return next })
      console.error('[StoryViewer] reactToStory error:', err?.message || err)
      if (err?.message?.includes('Unauthorized') || err?.message?.includes('not authenticated')) {
        toast.info('Sign in to react')
      } else if (err?.message?.includes('already reacted')) {
        toast.info(`Already reacted ${emoji}`)
      } else if (err?.message?.includes('not found') || err?.message?.includes('Story not found')) {
        toast.error('Story expired or unavailable')
      } else {
        toast.error(err?.message?.slice(0, 80) || 'Could not react')
      }
    }
  }
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      console.error('[StoryViewer] reply error:', err?.message || err)
      if (err?.message?.includes('Unauthorized') || err?.message?.includes('not authenticated')) {
        toast.info('Sign in to reply')
      } else {
        toast.error(err?.message?.slice(0, 80) || 'Could not send reply')
      }
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm flex items-center justify-center">
      ...
      >
        ...
        {/* Reaction flash — large emoji that fades out in center of screen */}
        {reactionFlash && (
          <div className="absolute inset-0 z-[60] flex items-center justify-center pointer-events-none">
            <span className="text-7xl animate-bounce opacity-80" style={{ animationDuration: '0.6s' }}>{reactionFlash}</span>
          </div>
        )}
        ...
        {/* Hidden ReactPlayer for embed audio (YouTube/Spotify in caption over image) */}
        {hasEmbedAudio && captionEmbedUrl && (
          <div style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', opacity: 0, pointerEvents: 'none' }}>
            <ReactPlayer
              ...
              onEnded={goToNextStory}
              config={{
                youtube: { playerVars: { autoplay: 1, playsinline: 1 } },
              }}
            />
          </div>
        )}
        ...
        {/* Embed audio badge (YouTube/Spotify from caption) */}
        {hasEmbedAudio && !currentStory.attachedTrack && (
          <div
            ...
            onClick={(e) => { e.stopPropagation(); setIsMuted(prev => !prev) }}
          >
            <div className="relative w-10 h-10 rounded bg-neutral-800 flex items-center justify-center">
              {isMuted ? <VolumeX className="w-5 h-5 text-white/60" /> : <Volume2 className="w-5 h-5 text-cyan-400" />}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-white text-xs font-medium truncate">
                {/youtu/i.test(captionEmbedUrl!) ? 'YouTube' : /spotify/i.test(captionEmbedUrl!) ? 'Spotify' : /soundcloud/i.test(captionEmbedUrl!) ? 'SoundCloud' : 'Music'}
              </p>
              <p className="text-neutral-400 text-[10px] truncate">{captionEmbedUrl}</p>
            </div>
            {!isMuted ? (
        ...
        {/* Attached track badge (if has NFT music) — tap toggles mute, tap title goes to profile */}
        {currentStory.attachedTrack && (
          ...
          >
            ...
            >
              ...
              <p className="text-cyan-400 text-[10px] truncate underline cursor-pointer active:text-cyan-300">{currentStory.attachedTrack.artist || 'Unknown Artist'} — View post ↗</p>
            </div>
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={async () => {
                    const url = getShareUrl()
                    if (navigator.share) {
                      try { await navigator.share({ title: 'SoundChain Reel', url }) } catch {}
                    } else {
