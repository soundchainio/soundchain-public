...
import { X, Lock, Coins, AlertCircle, CheckCircle, Loader2, Wallet, ChevronDown, ChevronUp, Sparkles } from 'lucide-react'
import { config } from '../../config'
import { useMagicContext } from 'hooks/useMagicContext'
import { useUnifiedWallet } from 'contexts/UnifiedWalletContext'
import { useMutation } from '@apollo/client'
import { MAKE_STORY_PERMANENT_MUTATION } from 'lib/graphql/mutations'
...
interface MakeStoryPermanentModalProps {
  isOpen: boolean
  onClose: () => void
  storyId: string
...
export const MakeStoryPermanentModal = ({
  isOpen,
  onClose,
  storyId,
  mediaSize = 0,
  onSuccess,
}: MakeStoryPermanentModalProps) => {
  ...
  }, [availableWallets.length, pricing.total, paymentToken, selectedWallet, availableWallets])
  ...
  const [makeStoryPermanent] = useMutation(MAKE_STORY_PERMANENT_MUTATION, {
    refetchQueries: ['publicStories', 'myFollowingStories', 'userStories'],
  })
  ...
  const handleMakePermanent = async () => {
    ...
    try {
      ...
      // Call the GraphQL mutation to mark story as permanent and generate SCid
      const result = await makeStoryPermanent({
        variables: {
          input: {
            storyId,
            paymentToken,
      ...
      if (result.data?.makeStoryPermanent?.success) {
        setStep('complete')
        const scid = result.data?.makeStoryPermanent?.story?.scid
        if (scid) {
          toast.success(`Reel is now permanent! SCid: ${scid}`)
        } else {
          toast.success('Reel is now permanent!')
        }
        setTimeout(() => {
          onSuccess?.()
          onClose()
        ...
        throw new Error(result.data?.makeStoryPermanent?.error || 'Failed to make story permanent')
      }
    } catch (error: any) {
  ...
  if (step === 'fee' || step === 'payment' || step === 'complete') {
    return (
      <div className="fixed bottom-4 left-4 right-4 z-[110] sm:left-auto sm:right-4 sm:w-80">
        <div className="bg-neutral-900/95 backdrop-blur-xl border border-neutral-700 rounded-2xl shadow-2xl p-4">
          {step === 'complete' ? (
            <div className="flex items-center gap-3">
              <CheckCircle className="w-6 h-6 text-green-400 flex-shrink-0" />
              <div>
                <p className="text-white font-medium text-sm">Reel is permanent!</p>
                <p className="text-neutral-500 text-xs">Earning streaming rewards via SCid</p>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3">
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-[110] sm:left-auto sm:right-4 sm:w-96">
      <div className="bg-neutral-900/95 backdrop-blur-xl border border-neutral-700 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-medium text-sm">Make Reel Permanent</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-3 space-y-3">
          {/* SCid Benefit Banner */}
          <div className="flex items-center gap-2 p-2 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border border-cyan-500/20 rounded-lg">
            ...
            <p className="text-cyan-400 text-xs">
              Permanent reels earn OGUN streaming rewards via SCid!
            </p>
          </div>
...
export default MakeStoryPermanentModal
