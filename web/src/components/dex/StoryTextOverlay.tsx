...
const DraggableText = ({
  ...
  const isDraggingRef = useRef(false) // Ref mirror to avoid stale closures in touch handlers
  const [isEditing, setIsEditing] = useState(!!autoEdit)
  const dragStart = useRef({ x: 0, y: 0, layerX: 0, layerY: 0 })
  // Double-tap detection for touch devices (onDoubleClick doesn't fire on iOS)
  const lastTapTime = useRef(0)
  // Track whether touch moved (don't start drag on pure tap)
  const touchMoved = useRef(false)
  // Stable ref for onUpdate to avoid re-creating touch handler
  ...
  // Touch support — preventDefault stops background page from scrolling while dragging text
  ...
  // Touch handlers use refs to avoid stale closures + useEffect re-runs mid-drag
  const handleTouchMove = useCallback((e: TouchEvent) => {
    e.preventDefault() // Prevent background scroll while dragging text

    const touch = e.touches[0]
    const dx = Math.abs(touch.clientX - dragStart.current.x)
    const dy = Math.abs(touch.clientY - dragStart.current.y)

    // Only start drag after moving 5px (prevents accidental drags on tap)
    ...
    if (!isDraggingRef.current && !touchMoved.current) return
  ...
  }, [containerRef]) // Stable: uses refs for isDragging and onUpdate

  const handleTouchEnd = useCallback(() => {
    isDraggingRef.current = false
    setIsDragging(false)

    // Double-tap detection: if touch didn't move and second tap within 300ms, enter edit mode
  ...
  // Attach touchmove/touchend at window level once and keep them stable
  useEffect(() => {
    ...
    <>
      ...
      {layers.map(layer => (
        <DraggableText
          ...
          autoEdit={layer.text === '' && selectedLayerId === layer.id}
        />
      ))}
