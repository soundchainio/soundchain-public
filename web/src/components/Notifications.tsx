...
export const Notifications: React.FC<SoundChainPopOverChildProps> = ({ closePopOver }) => {
  ...
  // Only reset badge count AFTER notifications have loaded — prevents the bug
  // where users tap the bell, see an empty panel, and the badge clears
  useEffect(() => {
  ...
  if (error) {
    ...
    return (
      <div className="flex h-[300px] w-full flex-col items-center justify-center p-8 gap-2">
        <h2 className="text-center font-semibold text-white">Could not load notifications</h2>
        <p className="text-xs text-gray-400 text-center">{error.message}</p>
      </div>
    )
  }
