...
@ObjectType()
export class WallAudioPlaylistItem {
  @Field(() => String)
  @prop({ required: true })
  audioUrl: string;
...
export class Profile extends Model {
  ...
  // Wall-uploaded audio as profile song (denormalized — no Track doc needed)
  @Field(() => String, { nullable: true })
  @prop({ required: false })
  ...
  // MySpace-style wall audio playlist (max 4 tracks, denormalized)
  @Field(() => [WallAudioPlaylistItem], { nullable: true })
  @prop({ required: false, type: () => [WallAudioPlaylistItem], default: [] })
  wallAudioPlaylist?: WallAudioPlaylistItem[];
