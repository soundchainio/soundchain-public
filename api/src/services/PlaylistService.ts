...
export class PlaylistService extends ModelService<typeof Playlist> {
  ...
  async addPlaylistItem(input: AddPlaylistItemInput, profileId: string): Promise<PlaylistTrack> {
    ...
    // Auto-fetch cover art for external URLs if no artworkUrl provided
    let resolvedArtworkUrl = artworkUrl || undefined;
    if (!resolvedArtworkUrl && externalUrl && sourceType !== PlaylistTrackSourceType.NFT) {
      try {
        const thumbnail = await fetchMediaThumbnail(externalUrl, externalUrl);
        if (thumbnail) {
          resolvedArtworkUrl = thumbnail;
          console.log('[PlaylistService] Auto-fetched cover art for:', externalUrl.substring(0, 60));
        }
      } catch (err) {
        // Non-blocking — cover art is optional
        console.warn('[PlaylistService] Failed to fetch cover art:', (err as Error).message);
      }
    }
