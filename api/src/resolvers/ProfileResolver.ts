...
export class ProfileResolver {
  ...
  async teamMember(@Ctx() { userService }: Context, @Root() profile: Profile): Promise<boolean> {
    try {
      ...
      // Orphaned profile (no matching User doc) — treat as non-team-member
      return false;
    }
  }
  ...
  async topFriendsProfiles(
    ...
    const profiles = await Promise.all(
      ids.slice(0, 15).map((id: string): Promise<Profile | null> =>
  ...
  async featuredTrack(@Root() profile: Profile): Promise<Track | null> {
    // Only return a track if the user explicitly set one
