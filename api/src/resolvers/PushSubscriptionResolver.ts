...
export class PushSubscriptionResolver {
  ...
  async testPushNotification(
    @CurrentUser() { profileId }: User,
    @Ctx() { webPushService }: Context,
  ): Promise<boolean> {
    try {
      await webPushService.sendNotification(profileId.toString(), {
        title: 'SoundChain Pulse',
        body: 'Push notifications are working! 🎵',
        data: { url: '/dex/pulse', type: 'test' },
      });
      return true;
    } catch (err) {
      console.error('[PushSubscriptionResolver] Test push failed:', err);
      return false;
    }
  }
}
