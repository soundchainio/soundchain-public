...
export class WallPostResolver {
  ...
  async createWallPost(
    ...
    @Arg('mediaTitle', { nullable: true }) mediaTitle: string,
  ...
  async updateWallPost(
    ...
    return wallPostService.updateWallPost(wallPostId, profileId.toString(), body);
  }
  ...
  @Query(() => WallPost, { nullable: true })
  async wallPost(
    @Ctx() { wallPostService }: Context,
    @Arg('id') id: string,
  ): Promise<WallPost | null> {
    return wallPostService.getWallPost(id);
  }
