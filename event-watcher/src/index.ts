...
import { ListingItemModel } from "./models/ListingItem";
import {
  ReceiptItemCanceled,
  ReceiptItemListed,
  ReceiptItemUpdated,
...
const db = {
  ...
  options: {
    ...
    sslCA:
      DATABASE_SSL_PATH &&
      fs.readFileSync(`${__dirname}/${DATABASE_SSL_PATH}`).toString(),
    retryWrites: false,
  },
};
...
marketplaceContract.events
  .ItemListed({})
  .on("data", async (receipt: ReceiptItemListed) => {
    const { owner, nft, tokenId, quantity, pricePerItem, startingTime } =
      receipt.returnValues;
...
marketplaceContract.events
  .ItemSold({})
  .on("data", async (receipt: ReceiptSold) => {
    const { seller, nft, tokenId, quantity, pricePerItem } =
      receipt.returnValues;
    console.log(receipt);
...
marketplaceContract.events
  .ItemUpdated({})
  .on("data", async (receipt: ReceiptItemUpdated) => {
    const { tokenId, newPrice } = receipt.returnValues;
    await ListingItemModel.findOneAndUpdate(
      {
        tokenId,
        valid: true,
...
marketplaceContract.events
  .ItemCanceled({})
  .on("data", async (receipt: ReceiptItemCanceled) => {
    const { tokenId } = receipt.returnValues;
    await ListingItemModel.findOneAndUpdate(
      {
        tokenId,
        valid: true,
